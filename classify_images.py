...
#                                                                             
# PROGRAMMER: Aakanksha Singh
# DATE CREATED: 23/11/2024                                
# REVISED DATE:  23/11/2024
...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       ...
       # as classifier label
        ...
        # Running the classifier function to classify the image based on the full path
        image_path = images_dir + '/' + key  # Full path for the image
        model_label = classifier(image_path, model)  # classifier function called here
       ...
       # set labels to lowercase (lower) and stripping off whitespace(strip)
        model_label = model_label.lower().strip()  # Process model label
  
        # Get the actual pet label from results_dic
    ...
    #           to add the classifier label (model_label) and the value of
       ...
       # results_dic as an exact match(1) using extend list function
        if truth in model_label:
            results_dic[key].extend([model_label, 1])  # Add model_label and a match (1)
        else:
       ...
       # the extend function 
            results_dic[key].extend([model_label, 0])  # Add model_label and no match (0)

    # Optional: Print the results dictionary to verify the output
    print(results_dic)
